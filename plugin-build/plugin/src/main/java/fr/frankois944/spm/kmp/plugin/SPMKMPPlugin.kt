...
public abstract class SPMKMPPlugin : Plugin<Project> {
    override fun apply(target: Project): Unit =
        with(target) {
           ...
           /* val targetToTaskName =
                CompileTarget.entries.associateWith {
                    getTaskName(name, it)
                }

            project.tasks.withType(CInteropProcess::class.java).configureEach { cinterop ->
                val cinteropTarget =
                ...
                val task =
                    tasks
                        .withType(GenerateCInteropDefinitionTask::class.java)
                        .findByName(taskName)
                        ?: return@configureEach

                cinterop.settings.definitionFile.set(task.outputFiles[0])
                cinterop.dependsOn(secondStep)
            }*/
        }
